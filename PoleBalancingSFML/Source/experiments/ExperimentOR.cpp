...
float ExperimentOR::evaluate(hn::HyperNet &hypernet, const hn::Config &config, std::mt19937 &generator) {
	float inputs[4][2] {
		{ -10.0f, -10.0f },
		{ -10.0f, 10.0f },
		{ 10.0f, -10.0f },
	...
	std::cout << "Finished OR experiment with total reward of " << (reward - initReward) * std::min(1.0f, std::max(0.0f, (reward * reward))) << "." << std::endl;
